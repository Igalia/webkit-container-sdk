...
printf "${application_name}: Build ubuntu_package_proxy image.\n\n"
...
if [ ${verbose} -eq 1 ]; then
    printf "   Build log:\n"
    podman build --jobs $(nproc --ignore=2) --tag docker.io/nikolaszimmermann/ubuntu-package-proxy:22.10 ${@} .
else
    podman build --jobs $(nproc --ignore=2) --tag docker.io/nikolaszimmermann/ubuntu-package-proxy:22.10 ${@} . &>/dev/null
fi
